...
#define ar_full_usage "\n\n" \
       "Extract or list FILEs from an ar archive"IF_FEATURE_AR_CREATE(", or create it")"\n" \
     "\n	x	Extract" \
     "\n	p	Extract to stdout" \
     "\n	t	List" \
	IF_FEATURE_AR_CREATE( \
     "\n	r	Create" \
	) \
     "\n	-o	Restore mtime" \
     "\n	-v	Verbose" \
...
#define bzip2_trivial_usage \
       "[-cfk" IF_FEATURE_BZIP2_DECOMPRESS("dt") "123456789] [FILE]..." \
...
#define tar_trivial_usage \
	...
	"vokO] " \
	"[-f TARFILE] [-C DIR] " \
	IF_FEATURE_TAR_FROM("[-T FILE] [-X FILE] "IF_FEATURE_TAR_LONG_OPTIONS("[OPTION]... ")) \
	"[FILE]..." \
...
#define chgrp_trivial_usage \
       "[-Rh"IF_DESKTOP("LHPcvf")"]... GROUP FILE..." \
...
#define stat_trivial_usage \
       "[-lt"IF_FEATURE_STAT_FILESYSTEM("f")"] "IF_FEATURE_STAT_FORMAT("[-c FMT] ")"FILE..." \
...
#define base32_trivial_usage \
	"[-d] [-w COL] [FILE]" \

#define base32_full_usage "\n\n" \
...
#define patch_trivial_usage \
       "[-RNE] [-p N] [-i DIFF] [ORIGFILE [PATCHFILE]]" \
...
#define vi_trivial_usage \
       IF_FEATURE_VI_COLON("[-c CMD] ")IF_FEATURE_VI_READONLY("[-R] ")"[FILE]..." \
...
#define cryptpw_trivial_usage \
       "[-s] [-p N] [-m TYPE] [-S SALT] [PASSWORD] [SALT]" \
...
#define adjtimex_trivial_usage \
       "[-q] [-o OFS] [-f FREQ] [-p TCONST] [-t TICK]" \
...
#define i2ctransfer_trivial_usage \
       "[-fay] I2CBUS {rLENGTH[@ADDR] | wLENGTH[@ADDR] DATA...}..." \

#define i2ctransfer_full_usage "\n\n" \
...
#define mim_trivial_usage \
	"[-f FILE] [SHELL_OPTIONS] [TARGET] ..." \

#define mim_full_usage "\n\n" \
...
#define insmod_trivial_usage \
	IF_FEATURE_2_4_MODULES("[-fkvqLx] MODULE") \
...
#define ifconfig_trivial_usage \
	IF_FEATURE_IFCONFIG_STATUS("[-a]") " [IFACE] [ADDRESS]" \
...
#define ifup_trivial_usage \
       "[-n"IF_FEATURE_IFUPDOWN_MAPPING("m")"vf] [-i FILE] -a | IFACE..." \
...
#define ipcalc_trivial_usage \
       "[-bnm"IF_FEATURE_IPCALC_FANCY("phs")"] ADDRESS" \
...
#define route_trivial_usage \
       "[-ne]"IF_FEATURE_IPV6(" [-A inet[6]]")" [{add|del} [-net|-host] TARGET [netmask MASK]\n" \
...
#define traceroute_trivial_usage \
       "[-"IF_TRACEROUTE6("46")IF_FEATURE_TRACEROUTE_USE_ICMP("I")"Flnrv] [-f 1ST_TTL] [-m MAXTTL] [-q PROBES] [-p PORT]\n" \
...
#define traceroute6_trivial_usage \
       "[-"IF_FEATURE_TRACEROUTE_USE_ICMP("I")"nrv] [-f 1ST_TTL] [-m MAXTTL] [-q PROBES] [-p PORT]\n" \
       "	[-t TOS] [-w WAIT_SEC] [-s SRC_IP] [-i IFACE]\n" \
       "	[-z PAUSE_MSEC] HOST [BYTES]" \
...
#define tunctl_trivial_usage \
       "[-f DEVICE] [-t NAME | -d NAME]" IF_FEATURE_TUNCTL_UG(" [-u USER] [-g GRP] [-b]") \
...
#define wget_trivial_usage \
	...
	IF_NOT_FEATURE_WGET_LONG_OPTIONS( \
       "[-cqS] [-O FILE] [-o LOGFILE] [-Y on/off] [-P DIR] [-U AGENT]"IF_FEATURE_WGET_TIMEOUT(" [-T SEC]")" URL..." \
	) \
...
#define fuser_trivial_usage \
       "[-msk46] [-SIGNAL] FILE or PORT/PROTO" \
...
#define pidof_trivial_usage \
       IF_FEATURE_PIDOF_SINGLE("[-s] ")IF_FEATURE_PIDOF_OMIT("[-o PID] ")"[NAME]..." \
...
#define top_trivial_usage \
       "[-b"IF_FEATURE_TOPMEM("m")IF_FEATURE_SHOW_THREADS("H")"]" \
       " [-n COUNT] [-d SECONDS]" \
...
#define hexdump_trivial_usage \
       "[-bcdoxCv] [-e FMT] [-f FMT_FILE] [-n LEN] [-s OFS] [FILE]..." \
...
#define umount_trivial_usage \
       "[-rlf"IF_FEATURE_MTAB_SUPPORT("m")IF_FEATURE_MOUNT_LOOP("d")IF_FEATURE_UMOUNT_ALL("a")"] [-t FSTYPE] FILESYSTEM|DIRECTORY" \
...
#define udhcpc6_trivial_usage \
       ...
       "	[-x OPT:VAL]... [-O OPT]..." IF_FEATURE_UDHCP_PORT(" [-P PORT]") \
